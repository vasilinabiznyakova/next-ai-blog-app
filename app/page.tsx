...
//the bellow switch on background revalidation for every 60sec
export const revalidate = 60;

//this is how we grab the table from db for static info, will add prerendering/cashing and gonna improve the perfomance
const getPosts = async () => {
  const posts = await prisma.post.findMany();

  const formattedPosts = await Promise.all(
    posts.map(async (post: Post) => {
      //need to modify the path from string to static path so that we could use placeholder="blur" for them
...
//by doing async we create a server component
